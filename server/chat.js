...
import { currentStepMessages, hasCompletedNodeTask, hasPendingQuiz } from "./learnGate.js";
...
## 教学节奏（强制关卡）
- 每一节（当前步骤）必须有且仅在收尾给出 **1 道节点任务 quiz**；未完成节点任务时用户不能进入下一节。
- 进入新步骤的开场：老师先讲 2~4 段 speech；需要时穿插 citation；本轮若尚未出过节点题，应在讲解末尾给出 quiz（quiz 必须是本轮最后一条）。
- 用户自由发言/快捷操作后：先围绕当前步骤回应；若本节还没有节点 quiz，本轮末尾必须补上。
- 用户刚答完节点题：先点评（对错与原因），可短续讲；不要立刻再出题。此时 advanceReady 可为 true。
- 严格围绕当前步骤，不要跳步。节点任务不可跳过。
...
advanceReady：仅当本节节点任务 quiz 已被用户正式作答后才可为 true；未作答或仍在讲解中必须为 false。
...
async function generateLearnSequence(chat, characters, messages, opts = {}) {
  ...
  const stepMsgs = currentStepMessages(chat);
  const nodeDone = hasCompletedNodeTask(chat);
  const nodePending = hasPendingQuiz(chat);
  const stepHasQuiz = stepMsgs.some((m) => (m.kind || "") === "quiz");
  const gateNote =
    `【关卡状态】本节节点任务：${
      nodePending ? "待作答" : nodeDone ? "已完成" : stepHasQuiz ? "已有题但未完成" : "尚未出题"
    }。未完成节点任务时 advanceReady 必须为 false。`;

  let instruction = "";
  if (opts.mode === "turn") {
    instruction =
      `【本轮任务：步骤开场】原因：${opts.reason || "start_step"}。\n` +
      `请老师先讲 2~4 段 speech 打开当前步骤；需要时插入 citation；本轮末尾必须给出 1 道节点任务 quiz（quiz 必须是最后一条）。Partner 最多插 1 句。不要等用户先说话。\n` +
      gateNote;
  } else if (opts.mode === "quiz_followup") {
    const meta = opts.quizMeta || {};
    instruction =
      `【本轮任务：节点任务点评】用户已正式作答本节节点测验（不可跳过）。\n` +
      `题目：${meta.prompt || ""}\n参考答案：${meta.answer || "（无）"}\n用户作答：${meta.response || ""}\n` +
      `请老师先点评对错与原因，再短续讲收束本节；本轮不要再出 quiz。advanceReady 应为 true。\n` +
      gateNote;
  } else if (opts.mode === "action") {
    instruction =
      `【本轮任务：快捷操作】${ACTION_HINTS[opts.action] || opts.action}\n` +
      (opts.extraText ? `用户补充：${opts.extraText}\n` : "") +
      (nodeDone || nodePending
        ? "本节节点任务已发出或已完成：本轮不要再出 quiz。\n"
        : "本节尚未完成节点任务：本轮讲解后必须在末尾给出 1 道节点 quiz。\n") +
      gateNote;
  } else if (opts.mode === "endless") {
    instruction =
      `【本轮任务：继续讲解】用户未发言。请老师继续当前步骤讲一小段（2~4 条），可穿插 citation；不要跳步。\n` +
      (nodeDone || nodePending
        ? "本节节点任务已发出或已完成：不要再出 quiz。\n"
        : "本节还没有节点任务：本轮末尾必须给出 1 道节点 quiz。\n") +
      gateNote;
  } else {
    instruction =
      `【本轮任务：回应用户】请以老师为主、partner 适量配合，围绕当前步骤回应。可穿插 citation。\n` +
      (nodeDone || nodePending
        ? "本节节点任务已发出或已完成：本轮不要再出 quiz。\n"
        : "本节尚未完成节点任务：本轮末尾必须给出 1 道节点 quiz。\n") +
      gateNote;
  }
